...
namespace sferes {
  namespace stat {
	...
	return (offset / m.strides()[dir] % m.shape()[dir] +  m.index_bases()[dir]);
      ...
      template<typename A>
	std::array<typename A::index, A::dimensionality> get_indexarray(const A& m, const typename A::element* e) {
	...
	std::vector<binary_map::Elem> _multi_array_to_vector(const A& array, const EA& ea) {
