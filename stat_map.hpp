...
public:
  ...
  typedef boost::array<typename array_t::index, Params::ea::behav_dim>
      behav_index_t;
  ...
  template <typename E> void refresh(const E &ea) {
    ...
    for (const phen_t *i = ea.archive().data();
         i < (ea.archive().data() + ea.archive().num_elements()); ++i) {
      phen_t p = *i;
...
protected:
  ...
  void _write_parents(const array_t &array, const array_t &p_array,
    ...
    for (const phen_t *i = array.data();
         i < (array.data() + array.num_elements()); ++i) {
      if (*i) {
  ...
  template <typename EA>
  void _write_archive(const array_t &array, const std::string &prefix,
                      const EA &ea) const {
    std::cout << "writing..." << prefix << ea.gen() << std::endl;
    std::string fname = ea.res_dir() + "/" + prefix +
                        boost::lexical_cast<std::string>(ea.gen()) +
  ...
  void _write_progress(const EA &ea, std::ofstream &ofs) const {
    ...
    for (const phen_t *i = array.data();
      ...
      if (*i) {
        ...
        for (size_t i = 0; i < Params::ea::behav_shape_size(); ++i) {
#ifdef NORMALIZED
          assert(p->fit().desc_normalized()[i] >= 0.0f &&
                 p->fit().desc_normalized()[i] <= 1.0f);

          float diff = p->fit().desc_normalized()[i] -
                       (float)round(p->fit().desc_normalized()[i] *
                                    (float)(Params::ea::behav_shape(i) - 1)) /
                           (float)(Params::ea::behav_shape(i) - 1);
#else
          assert(p->fit().desc()[i] >= 0.0f && p->fit().desc()[i] <= 1.0f);

          float diff = p->fit().desc()[i] -
                       (float)round(p->fit().desc()[i] *
                                    (float)(Params::ea::behav_shape(i) - 1)) /
                           (float)(Params::ea::behav_shape(i) - 1);
#endif
    ...
    ofs << ea.gen() << " " << archive_size << " " << archive_mean << " "
        << archive_max << " " << mean_dist_center << std::endl;
  }
};
}
}
